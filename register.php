...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validazione input (username sito obbligatorio, email obbligatoria)
    if (empty($minecraft_nick) || empty($email) || empty($password) || empty($password_confirm)) {
        $error = 'Per favore, inserisci username del sito, email e password.';
    } elseif (strlen($minecraft_nick) < 3 || strlen($minecraft_nick) > 16) {
        $error = 'Lo username deve essere tra 3 e 16 caratteri.';
        ...
        $stmtChk = $pdo->prepare("SELECT id FROM sl_users WHERE email = ? LIMIT 1");
            ...
            <div class="col-12">
                <div class="auth-card">
                    <div class="auth-header">
                        ...
                        <p class="auth-subtitle">Registrati per iniziare a votare e partecipare al forum</p>
                    </div>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label for="minecraft_nick" class="form-label">
                                            <i class="bi bi-person"></i> Username del sito (obbligatorio)
                                        </label>
                                        <input type="text" class="form-input" id="minecraft_nick" name="minecraft_nick" 
                                               ...
                                               placeholder="Scegli il tuo username" maxlength="16">
                                        <div class="form-hint">3-16 caratteri (lettere, numeri, underscore)</div>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label for="email" class="form-label">
                                            <i class="bi bi-envelope"></i> Email (obbligatoria)
                                        </label>
...
document.getElementById('registerForm').addEventListener('submit', function(e) {
    ...
    if (minecraftNick.length < 3 || minecraftNick.length > 16) {
        e.preventDefault();
        showAuthToast('Lo username deve essere tra 3 e 16 caratteri.', 'error');
        return false;
    }
    
    if (!/^[a-zA-Z0-9_]+$/.test(minecraftNick)) {
        e.preventDefault();
        showAuthToast('Lo username può contenere solo lettere, numeri e underscore.', 'error');
        return false;
    }
    
    if (email.length === 0 || !/^\S+@\S+\.\S+$/.test(email)) {
        e.preventDefault();
        showAuthToast('Inserisci un indirizzo email valido.', 'error');
        return false;
    }
